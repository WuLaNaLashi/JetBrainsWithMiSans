...
# 清理并创建输出目录
echo "准备输出目录..."
if [ -d "MiWithJBMonoNL" ]; then
    rm -rf "MiWithJBMonoNL"
fi
mkdir -p "MiWithJBMonoNL"

# 步骤1: 使用FontForge处理字体
echo "步骤1: 使用FontForge处理字体..."
fontforge -script merge_fonts_ff.py
...
echo ""
echo "生成的字体文件:"
ls -la MiWithJBMonoNL/*.ttf
echo ""

# 统计生成的文件数量
file_count=$(find MiWithJBMonoNL -name "*.ttf" | wc -l)
...
echo ""
echo "字体文件详情:"
for file in MiWithJBMonoNL/*.ttf; do
    if [ -f "$file" ]; then
        echo "----------------------------------------"
