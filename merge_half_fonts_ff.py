...
[MiWithJBMonoHalf]
Copyright (c) 2025, MiWithJBMonoHalf Project
"""
...
def generate_font(zh_style, eng_style, output_style, eng_font_path, font_suffix):
    ...
    # 调整一些字形
    adjust_some_glyph(zh_font, eng_font, output_style)
    ...
    # 转换半角宽度
    transform_half_width(zh_font, eng_font)
...
def adjust_some_glyph(zh_font, eng_font, style="Regular"):
    """调整一些字形"""
    try:
    ...
    try:
        if not zh_font[0x3042].isWorthOutputting():
            return
        full_width = zh_font[0x3042].width
    except:
        return
    ...
    # 调整全角句号和逗号
    try:
        for glyph in zh_font.selection.select(("unicode", None), 0xFF0E).byGlyphs:  # ．
            glyph.transform(psMat.scale(1.2, 1.2))
            glyph.width = full_width
        
        for glyph in zh_font.selection.select(("unicode", None), 0xFF0C).byGlyphs:  # ，
...
def transform_half_width(zh_font, eng_font):
    """转换半角宽度"""
    try:
