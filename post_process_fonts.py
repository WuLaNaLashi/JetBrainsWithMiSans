...
def process_all_fonts():
    ...
    for eng_file in eng_files:
        ...
        if len(parts) >= 2:
            font_variant = parts[0]  # MiWithJBMono 或 MiWithJBMonoNL
...
def merge_fonts(eng_path, zh_path, output_path):
    """合并字体"""
    try:
        print(f"合并字体: {eng_path} + {zh_path}")
        
        # 删除中文字体的vhea和vmtx表以避免冲突
        zh_font = ttLib.TTFont(zh_path)
        if "vhea" in zh_font:
            del zh_font["vhea"]
        if "vmtx" in zh_font:
            del zh_font["vmtx"]
        zh_font.save(zh_path)
        
        # 使用fonttools合并字体
        merger = merge.Merger()
        merged_font = merger.merge([eng_path, zh_path])
    ...
    try:
        ...
        if name_table is not None:
            for namerecord in name_table.findall("namerecord[@nameID='0']"):
                if namerecord.text and "Plemolzh" in namerecord.text:
